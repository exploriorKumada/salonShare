...
public class GameManager : MonoBehaviour
{
    ...
    [NonSerialized] int keyAddMaxCount = 1000;//鍵の追加アイテムによる最大数　これ以上は追加アイテムを拾っても追加されない
    ...
    [NonSerialized] public int stageCount = 1;

    public int bournus()
        {
            return stageCount / 5 + addBournus;
        }


    public int addBournus { get; private set; } = 0;
    ...
    public void SetTextAnimator(string value, float second, bool isRoop)
    {
        ...
        textAnimatorPlayer.transform.localScale = Vector3.one;
    ...
    public void GetKey(KeyKind keyKind)
    {
        ...
        keyGetCount = keyGetCount + 1 + bournus();
    ...
    private void Update()
    {
        ...
        menu.stageCountText.text = stageCount.ToString();

        menu.bournusText.text = "×" + (bournus()+1);
    ...
    {
        menu.timerText.text = ((int)(nowsecond / 60)).ToString("00") + ":" + ((int)(nowsecond % 60)).ToString("00");
    }
    ...
    public void AddBournus(int add)
    {
        if (add == 0) return;

        SetTextAnimator("Bournus! +" + add, 2, false);

        addBournus = addBournus + add;
    }
